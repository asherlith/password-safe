...
class PasswordAPI(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        user_pwd = request.data.get("pwd").encode()
        user = authenticate(request, username=request.user, password=user_pwd)
        if user:
        ...
        return Response(data={"error": "wrong password"}, status=status.HTTP_401_UNAUTHORIZED)
